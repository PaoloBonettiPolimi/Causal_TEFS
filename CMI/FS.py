...
def scoreFeatures(features, target, k, tau, selected=None):
    ...
    for col in range(features.shape[1]):
        ...
        candidate = features[tau-1:-1, col].reshape(-1,1)
        conditioning = sel[tau-1:-1, :]
...
def backwardFeatureSelection(threshold,features,target,res,k, nproc, tau):
    ...
    while CMIScore < threshold and relevantFeatures.shape[1]>1: 
        featureScores = scoreFeatures(relevantFeatures, target, k, tau) # for each feature it evaluates the I(Y,X_i|X_A), at first step I(Y,X_i|X_{-i}),...
...
def forwardFeatureSelection(threshold,features,target,res,k, nproc, tau):
    ...
    while CMIScore < threshold and n_iters < features.shape[1]:
        featureScores = scoreFeatures(remainingFeatures, target, k, tau, np.array(selectedFeatures).T) 
